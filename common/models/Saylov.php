...
/**
 * This is the model class for table "saylov".
 *
 * @property int $id
 ...
 */
class Saylov extends \yii\db\ActiveRecord
{
    public $image;
    public $image1;
    /**
    ...
    public function afterDelete()
    {
        if ($this->photo !== null && !empty($this->photo)) {
            unlink(Yii::getAlias('@frontend').'/web/uploads/saylov/' . $this->photo);
        }
    ...
    public function upload()
    {
        if ($this->validate()) {
            ...
            if ($this->main_photo !== null && !empty($this->main_photo)) {
                unlink(Yii::getAlias('@frontend').'/web/uploads/saylov/' . $this->main_photo);
            }
            if ($this->photo !== null && !empty($this->photo)) {
                unlink(Yii::getAlias('@frontend').'/web/uploads/saylov/' . $this->photo);
            }
            ...
            $this->photo = $name;
            $this->image->saveAs(Yii::getAlias('@frontend').'/web/uploads/saylov/' . $name);
            $this->image1->saveAs(Yii::getAlias('@frontend').'/web/uploads/saylov/' . $name1);
            return true;
        } else {
            return false;
        }
