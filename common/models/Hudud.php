...
/**
 * This is the model class for table "hudud".
 *
 * @property int $id
    ...
    public function upload()
    {
        if ($this->validate()) {
            ...
            if ($this->photo !== null && !empty($this->photo)) {
                unlink(Yii::getAlias('@frontend').'/web/uploads/hudud/' . $this->photo);
            }
            $this->photo = $name;
            $this->image->saveAs(Yii::getAlias('@frontend').'/web/uploads/hudud/' . $name);
            return true;
        } else {
            return false;
        }
