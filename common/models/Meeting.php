...
class Meeting extends \yii\db\ActiveRecord
{
    ...
    public function rules()
    {
        return [
            ...
            [['title_ru', 'title_uz', 'title_en', 'photo', 'date'], 'string', 'max' => 255],
    ...
    public function upload()
    {
        if ($this->validate()) {
            ...
            if ($this->photo !== null && !empty($this->photo)) {
                unlink(Yii::getAlias('@frontend').'/web/uploads/meeting/' . $this->photo);
            }
            $this->photo = $name;
            $this->image->saveAs(Yii::getAlias('@frontend').'/web/uploads/meeting/' . $name);
            return true;
        } else {
            return false;
        }
